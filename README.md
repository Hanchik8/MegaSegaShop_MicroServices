...
Каждый сервис отдаёт метрики по адресу `/actuator/prometheus`.  
Prometheus и Grafana поднимаются через профиль `observability`.

## Безопасность и обработка ошибок

- JWT проверяется в API Gateway, а роль пользователя пробрасывается в заголовке `X-User-Role`.
- Админские эндпоинты помечены `@AdminOnly` и требуют роль `ROLE_ADMIN` (допустимо `ADMIN` для сервисных вызовов).
- Общие ответы об ошибках формируются через `GlobalExceptionHandler`.
...
# Создание товара (требует ROLE_ADMIN)
